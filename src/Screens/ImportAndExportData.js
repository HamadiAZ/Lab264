...
import OfflineDataImpExp from "../Components/OfflineDataImpExp";
...
function ImportAndExportData() {
  ...
  async function isDeviceConnectedToInternet() {
  ...
  if (!isLoggedIn)
    return (
      <View className="flex-1 flex justify-center items-center gap-7">
        ...
        <OfflineDataImpExp setLoading={setLoading} loading={loading} />
      </View>
    );
