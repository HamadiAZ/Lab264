    ...
    <KeyboardAvoidingView>
      <View className="bg-gray-100 mt-1">
        <TouchableOpacity className="flex flex-row justify-between mb-0 pb-0">
          ...
          <View className="flex  align-baseline justify-center  h-14 rounded-sm mr-5 " onPress={""}>
            <Text className="text-l text-gray-400 mb-0 pb-0">{deviceState.state == "" ? "" : "=>" + deviceState.state}</Text>
          </View>
        </TouchableOpacity>
        <View className="flex flex-row px-3 h-9 justify-between align-middle mb-2">
          <TextInput className="border-b-2 border-gray-300 w-9/12" onChangeText={(text) => setMessage(text.toString())} value={message.toString()} />
